/*
 * Jetson Nano CUDA 3D Demo - Bouncing Ball in Spinning Cube
 *
...
__global__ void clearKernel(unsigned char* pixels, int width, int height) {
    ...
    pixels[idx + 0] = (unsigned char)(20 + t * 30);   // B
    pixels[idx + 1] = (unsigned char)(10 + t * 20);   // G
    pixels[idx + 2] = (unsigned char)(30 + t * 20);   // R
    pixels[idx + 3] = 255;
}
...
__global__ void drawShadowKernel(unsigned char* pixels, int width, int height,
    ...
    float dx = x - shadowX;
    float dy = (y - shadowY) * 2.0f;  // Squash for perspective
    float dist = sqrtf(dx * dx + dy * dy);
...
// Draw cube edges (called from host, uses single thread per edge)
__global__ void drawCubeEdgesKernel(unsigned char* pixels,
    ...
    // Depth-based alpha for transparency effect
...
int main() {
    printf("=== Jetson Nano CUDA 3D Demo ===\n");
    ...
    XStoreName(display, window, "CUDA 3D - Bouncing Ball in Cube");
    ...
    ball.color = make_vec3(1.0f, 0.3f, 0.1f);  // Orange-red
    ...
    while (1) {
        ...
        while (XPending(display)) {
            ...
            if (event.type == KeyPress) {
                ...
                if (key == XK_Left) rotY -= 0.2f;
                if (key == XK_Right) rotY += 0.2f;
                if (key == XK_Up) rotX -= 0.1f;
                if (key == XK_Down) rotX += 0.1f;
                if (key == XK_plus || key == XK_equal) camDist -= 0.5f;
                if (key == XK_minus) camDist += 0.5f;
