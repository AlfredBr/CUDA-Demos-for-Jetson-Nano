/*
 * Jetson Nano CUDA Cornell Box Path Tracer
 * 
...
int main() {
    ...
    XStoreName(display, window, "CUDA Cornell Box Path Tracer");
    ...
    while (1) {
        ...
        if (currentTime - lastFpsTime >= 0.5) {
            float fps = fpsFrameCount / (currentTime - lastFpsTime);
            float sps = totalSamples / (currentTime - startTime);
