/*
 * Windows CUDA Parallel Primitives Toybox
...
__global__ void renderParticlesKernel(unsigned char* pixels, const Particle* particles,
    ...
    // Fade background
    pixels[idx + 0] = (unsigned char)(pixels[idx + 0] * 0.95f);
    pixels[idx + 1] = (unsigned char)(pixels[idx + 1] * 0.95f);
    pixels[idx + 2] = (unsigned char)(pixels[idx + 2] * 0.95f);
    pixels[idx + 3] = 255;
}
...
int main() {
    ...
    // Win32 setup
    Win32Display* display = win32_create_window("CUDA Parallel Primitives Toybox", WIDTH, HEIGHT);
    if (!display) {
    ...
    initParticlesKernel<<<blocks, 256>>>(d_particles, numParticles, (unsigned int)time(NULL));
    ...
    while (running && !win32_should_close(display)) {
        ...
        while (win32_pop_event(display, &event)) {
            if (event.type == WIN32_EVENT_KEY_PRESS) {
                ...
                if (key == XK_2 && currentDemo != 2) {
                    ...
                    // Run prime sieve
                    double t0 = win32_get_time(display);
                    ...
                    double elapsed = win32_get_time(display) - t0;
                ...
                if (key == XK_plus || key == XK_equal) {
                    if (currentDemo == 1) {
                        ...
                        initParticlesKernel<<<(numParticles + 255) / 256, 256>>>(d_particles, numParticles, (unsigned int)time(NULL));
                        printf("Particles: %d\n", numParticles);
            ...
            if (event.type == WIN32_EVENT_CLOSE) running = false;
        }
        ...
        if (currentTime - lastFpsTime >= 1.0) {
            float fps = (float)(frameCount / (currentTime - lastFpsTime));
            ...
            // Note: Win32 doesn't have a direct title update in our minimal API
            // But we can print to console
            printf("%s\n", title);
