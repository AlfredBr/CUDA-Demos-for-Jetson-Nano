// Conway's Game of Life - CUDA Demo (Windows Port)
// Interactive cellular automaton with zoom/pan, painting, rule editor
// Uses Win32 + CUDA (no OpenGL)
...
// Additional key definitions not in win32_display.h
#define XK_bracketleft  VK_OEM_4
#define XK_bracketright VK_OEM_6
#define XK_Q            'Q'
#define XK_R            'R'
#define XK_C            'C'
#define XK_S            'S'
...
int main() {
    printf("=== Conway's Game of Life - CUDA Demo (Windows) ===\n");
    ...
    // Win32 setup
    Win32Display* display = win32_create_window("Conway's Game of Life - CUDA", WIDTH, HEIGHT);
    if (!display) {
    ...
    while (1) {
        // Handle events
        if (win32_process_events(display)) {
            break;
        }
        ...
        while (win32_pop_event(display, &event)) {
            if (event.type == WIN32_EVENT_KEY_PRESS) {
            ...
            else if (event.type == WIN32_EVENT_MOUSE_PRESS) {
                mouseDown = 1;
                ...
                dim3 paintGrid(1, 1);
                int value = (mouseButton == Button1) ? 1 : 0;
                paintKernel<<<paintGrid, paintBlock>>>(d_grid[currentGrid], gx, gy, state.brushSize, value);
            }
            else if (event.type == WIN32_EVENT_MOUSE_RELEASE) {
                mouseDown = 0;
            }
            else if (event.type == WIN32_EVENT_MOUSE_MOVE && mouseDown) {
                int mx = event.mouseX;
                int my = event.mouseY;
                int gx = (int)(state.panX + mx / state.zoom);
                int gy = (int)(state.panY + my / state.zoom);
        ...
        if (showHelp) {
            ...
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 10, 255, 255, 255);

            sprintf(buf, "Pop: %d", state.population);
            cudaMemcpy(d_text, buf, strlen(buf) + 1, cudaMemcpyHostToDevice);
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 25, 255, 255, 255);

            char ruleBuf[32];
            getRuleString(&state, ruleBuf);
            sprintf(buf, "Rule: %s", ruleBuf);
            cudaMemcpy(d_text, buf, strlen(buf) + 1, cudaMemcpyHostToDevice);
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 40, 100, 255, 100);

            sprintf(buf, "(%s)", presetNames[currentPreset]);
            cudaMemcpy(d_text, buf, strlen(buf) + 1, cudaMemcpyHostToDevice);
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 55, 100, 200, 255);

            sprintf(buf, "Zoom: %.1fx", state.zoom);
            cudaMemcpy(d_text, buf, strlen(buf) + 1, cudaMemcpyHostToDevice);
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 70, 200, 200, 200);

            sprintf(buf, "Speed: %d", state.speed);
            cudaMemcpy(d_text, buf, strlen(buf) + 1, cudaMemcpyHostToDevice);
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 85, 200, 200, 200);

            sprintf(buf, "Brush: %d", state.brushSize);
            cudaMemcpy(d_text, buf, strlen(buf) + 1, cudaMemcpyHostToDevice);
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 100, 200, 200, 200);

            sprintf(buf, "Wrap: %s", state.wrapEdges ? "ON" : "OFF");
            cudaMemcpy(d_text, buf, strlen(buf) + 1, cudaMemcpyHostToDevice);
            drawTextKernel<<<(strlen(buf) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(buf), 10, 115, 200, 200, 200);
            ...
            drawTextKernel<<<(strlen(statusText) * 48 + 255) / 256, 256>>>(d_pixels, d_text, (int)strlen(statusText), 10, 130, sr, sg, 100);
        }
