// Infinite Corridor - CUDA port for Windows
...
int main() {
    ...
    // Win32 setup
    Win32Display* display = win32_create_window("Infinite Corridor - CUDA", WIDTH, HEIGHT);
    if (!display) {
    ...
    while (running && !win32_should_close(display)) {
        ...
        while (win32_pop_event(display, &event)) {
            if (event.type == WIN32_EVENT_KEY_PRESS) {
                if (event.key == XK_q || event.key == XK_Escape) {
                    running = 0;
                } else if (event.key == XK_space) {
                    paused = !paused;
