/*
 * Windows CUDA 2D Fluid Simulation
...
int main() {
    printf("=== Windows CUDA 2D Fluid Simulation ===\n");
    ...
    // Create Win32 window
    Win32Display* display = win32_create_window("CUDA 2D Fluid Simulation - Stable Fluids",
    ...
    while (!win32_should_close(display)) {
        ...
        while (win32_pop_event(display, &event)) {
            ...
            if (event.type == WIN32_EVENT_MOUSE_RELEASE) {
                mouseDown = 0;
            }

            if (event.type == WIN32_EVENT_MOUSE_MOVE && mouseDown) {
                int mx = event.mouseX;
                int my = event.mouseY;
