/*
 * CUDA Perlin Noise Explorer (Windows Port)
...
__device__ void terrainRamp(float n, unsigned char* r, unsigned char* g, unsigned char* b) {
    n = fmaxf(0.0f, fminf(1.0f, (n + 1.0f) * 0.5f));
    if (n < 0.3f) { float t = n / 0.3f; *r = 20 + (unsigned char)(t * 40); *g = 50 + (unsigned char)(t * 80); *b = 120 + (unsigned char)(t * 60); }
    else if (n < 0.4f) { float t = (n - 0.3f) / 0.1f; *r = 60 + (unsigned char)(t * 140); *g = 130 + (unsigned char)(t * 70); *b = 180 - (unsigned char)(t * 100); }
    else if (n < 0.5f) { float t = (n - 0.4f) / 0.1f; *r = 200 - (unsigned char)(t * 140); *g = 200 - (unsigned char)(t * 50); *b = 80 - (unsigned char)(t * 50); }
    else if (n < 0.7f) { float t = (n - 0.5f) / 0.2f; *r = 60 + (unsigned char)(t * 60); *g = 150 - (unsigned char)(t * 50); *b = 30 + (unsigned char)(t * 40); }
    else if (n < 0.85f) { float t = (n - 0.7f) / 0.15f; *r = 120 + (unsigned char)(t * 40); *g = 100 + (unsigned char)(t * 40); *b = 70 + (unsigned char)(t * 50); }
    else { float t = (n - 0.85f) / 0.15f; *r = 160 + (unsigned char)(t * 95); *g = 140 + (unsigned char)(t * 115); *b = 120 + (unsigned char)(t * 135); }
}
...
__device__ void marbleRamp(float n, unsigned char* r, unsigned char* g, unsigned char* b) {
    ...
    *r = (unsigned char)(180 + marble * 75); *g = (unsigned char)(170 + marble * 85); *b = (unsigned char)(160 + marble * 95);
}
...
int main() {
    printf("=== CUDA Perlin Noise Explorer (Windows) ===\n\n");
    cudaDeviceProp prop;
    ...
    Win32Display* display = win32_create_window("CUDA Perlin Noise Explorer", WIDTH, HEIGHT);
    ...
    while (1) {
        ...
        while (win32_pop_event(display, &event)) {
            if (event.type == WIN32_EVENT_KEY_PRESS) {
                ...
                else if (key == XK_f) { params.frequency *= 1.1f; needsUpdate = true; }
                else if (key == XK_g) { params.frequency /= 1.1f; needsUpdate = true; }
                else if (key == XK_l) { params.lacunarity += 0.1f; needsUpdate = true; }
                else if (key == XK_k) { params.lacunarity = fmaxf(1.1f, params.lacunarity - 0.1f); needsUpdate = true; }
                else if (key == XK_p) { params.persistence = fminf(1.0f, params.persistence + 0.05f); needsUpdate = true; }
                else if (key == XK_o) { params.persistence = fmaxf(0.1f, params.persistence - 0.05f); needsUpdate = true; }
                else if (key == XK_w) { params.warpStrength += 0.1f; needsUpdate = true; }
                else if (key == XK_s) { params.warpStrength = fmaxf(0.0f, params.warpStrength - 0.1f); needsUpdate = true; }
                else if (key == XK_t) { params.timeSpeed += 0.1f; needsUpdate = true; }
                else if (key == XK_r) { params.timeSpeed = fmaxf(0.0f, params.timeSpeed - 0.1f); needsUpdate = true; }
                else if (key == XK_plus || key == XK_equal) { params.octaves = params.octaves < 10 ? params.octaves + 1 : 10; needsUpdate = true; }
                else if (key == XK_minus) { params.octaves = params.octaves > 1 ? params.octaves - 1 : 1; needsUpdate = true; }
                else if (key == XK_c) { params.colorMode = (params.colorMode + 1) % 4; needsUpdate = true; }
                else if (key == XK_n) { params.noiseMode = (params.noiseMode + 1) % 3; needsUpdate = true; }
                else if (key == XK_z) {
                    params.seed = rand();
                    generatePermutation(perm, params.seed);
                    cudaMemcpyToSymbol(d_perm, perm, 512);
                ...
                else if (key == XK_h) { showHelp = !showHelp; }
                else if (key == XK_space) {
                    paused = !paused;
                    if (paused) pausedTime = (float)(win32_get_time(display) - startTime);
                    else startTime = win32_get_time(display) - pausedTime;
                }
        ...
        float time = paused ? pausedTime : (float)(win32_get_time(display) - startTime);
        ...
        if (currentTime - lastFpsTime >= 0.5) {
            fps = (float)(frameCount / (currentTime - lastFpsTime));
            char title[128];
